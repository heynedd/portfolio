...
document.querySelectorAll('.detail-trigger').forEach((button) => {
  button.addEventListener('click', () => {
    ...
    updateDetailButtons();
    if (opening && !matchMedia('(prefers-reduced-motion: reduce)').matches && !doc.classList.contains('motion-paused')) {
...
applyLanguage(lang);
// V4: native motion.js owns reveals; no external animation runtime is required.
